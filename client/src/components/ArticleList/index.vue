<template>
  <div>
    <NuxtLink
      ...
      tag="section"
      :to="`/content/${item.id}`"
      class="md:duration-300 md:hover:scale-[1.02] md:flex mx-3 md:mx-0 min-h-[140px] shadow-3xl rounded-lg bg-white mb-5 cursor-pointer overflow-hidden"
    >
      ...
      <div :class="[ 'px-5', 'p-4', 'basis-2/3', 'flex', 'flex-col', { 'basis-full': !item.firstPicture }]">
        <div class="flex text-gray-500 text-xs md:mt-0">
          ...
          <div v-if="item.tags.length" class="info-item">
            <NuxtLink v-for="tag in item.tags" :key="tag.id" class="ml-2 art-tag" :to="{ name: 'search', params: { tagName: tag.name } }">
              {{ tag.name }}
            </NuxtLink>
          </div>
        </div>
